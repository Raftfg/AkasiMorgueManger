 ...
 * @return array
 */
if (!function_exists('parametre_api')) {

    function parametre_api() {
        return DB::table('parametres')->first();
    }
}

/**
 * Récupère l'id de l'exercice en vérifiant à quelle période d'un exercice la date correspond
 * 
 * @return array
 */
if (!function_exists('exercice_id_by_date')){

    function exercice_id_by_date($date){
        $date = date("Y-m-d", strtotime($date));
        return DB::table('exercices')->where('date_debut', '<=', $date)->where('date_fin', '>=', $date)->first()->id;
    }
}
